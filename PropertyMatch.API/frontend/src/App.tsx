import { useEffect, useState as import_useState } from 'react'
...
function ProtectedRoute({ children, roles, requireVerified = true }: ProtectedRouteProps) {
    ...
    if (requireVerified && user.status === 'Pending') {
        return (
            <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg)', padding: 24 }}>
                <div style={{ maxWidth: 440, textAlign: 'center', background: 'var(--bg-card)', borderRadius: 16, border: '1px solid var(--border)', padding: '40px 36px' }}>
                    <div style={{ fontSize: '3rem', marginBottom: 16 }}>📧</div>
                    <h2 style={{ fontSize: '1.2rem', fontWeight: 700, marginBottom: 10 }}>Verify your email first</h2>
...
function ResendFromBlockedPage({ email }: { email: string }) {
    const [sent, setSent] = import_useState(false)
    const [loading, setLoading] = import_useState(false)
    ...
    return (
        <div style={{ display: 'flex', gap: 8, justifyContent: 'center' }}>
            <button className="btn btn-outline"
                ...
                onClick={async () => {
                    setLoading(true)
                    try { await authVerifyApi.resend(email) } catch { /* always success */ }
                    setLoading(false); setSent(true)
                }}>
                {sent ? '✅ Email sent' : loading ? <span className="spinner" /> : 'Resend verification link'}
            </button>
            <a href="/login" className="btn btn-ghost">Sign out</a>
        </div>
    )
}
                ...
                <BrowserRouter>
                    <Routes>
                        ...
                        <Route path="/listing/:id" element={
                            <ProtectedRoute roles={['Tenant']} requireVerified={false}>
                                <AppShell><ListingDetailPage /></AppShell>
                            </ProtectedRoute>
                        } />
