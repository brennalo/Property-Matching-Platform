...
export default function TenantSchedulesPage() {
    ...
    return (
      <div className="card" style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
        ...
        <div style={{
          ...
          textAlign: 'center', flexShrink: 0, minWidth: 70,
        }}>
          <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)', textTransform: 'uppercase' }}>
            {date.split(',')[0]}
          </div>
          <div style={{ fontFamily: 'DM Serif Display, serif', fontSize: '1.6rem', lineHeight: 1 }}>
            {new Date(s.scheduledAt).getDate()}
          </div>
          <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
            {new Date(s.scheduledAt).toLocaleString('en-MY', { month: 'short' })}
          </div>
        </div>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: '0.82rem', color: 'var(--text-muted)', display: 'flex', flexDirection: 'column', gap: 3 }}>
            <span><MapPin size={12} style={{ verticalAlign: 'middle' }} /> {s.listingAddress}</span>
            <span><Clock size={12} style={{ verticalAlign: 'middle' }} /> {time}</span>
          </div>
        </div>
      </div>
    )
  ...
  if (isLoading) return <div style={{ textAlign: 'center', padding: 60 }}><span className="spinner" /></div>
    ...
    <div>
      ...
      {schedules.length === 0 ? (
        <div className="empty-state">
          ...
          <p style={{ marginTop: 4 }}>Find a property and book a viewing from the results page</p>
        </div>
      ) : (
        <>
          {upcoming.length > 0 && (
            <div style={{ marginBottom: 28 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {upcoming.map((s, i) => <ScheduleCard key={i} s={s} />)}
              </div>
            </div>
          )}
          ...
          {past.length > 0 && (
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10, opacity: 0.6 }}>
                {past.map((s, i) => <ScheduleCard key={i} s={s} />)}
              </div>
            </div>
          )}
        </>
