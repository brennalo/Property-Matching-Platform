...
function LifestyleMapCard({
    ...
    useEffect(() => {
        ...
        allPlaces
            .filter((p) => activeTypes.size === 0 || activeTypes.has(p.type))
            .forEach((p) => {
    ...
    const toggleType = (type: string) => {
        setActiveTypes((prev) => {
            // if already the only active → clear (show all)
            if (prev.size === 1 && prev.has(type)) return new Set<string>();
            return new Set([type]);
        });
    };
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                    <div>
                        ...
                        >
                            ...
                            {!fromBrowse && <ScoreRing score={result.totalScore} />}
                        </div>
                        <div
                            style={{
                                display: "flex",
