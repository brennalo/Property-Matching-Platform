...
function CalendarGrid({
  year, month, schedules, onDayClick, selectedDay,
}: {
  year: number
  month: number
  ...
  return (
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 4, marginBottom: 8 }}>
        {dayLabels.map(d => (
          <div key={d} style={{ textAlign: 'center', fontSize: '0.72rem', color: 'var(--text-dim)', fontWeight: 600 }}>
            {d}
          </div>
        ))}
      </div>
            ...
            <button key={i} onClick={() => onDayClick(day)}
              style={{
                ...
                background: isSelected ? 'var(--accent)'
                  : isToday ? 'rgba(232,160,69,0.15)'
                  : 'var(--bg-input)',
                color: isSelected ? '#0f0f0e'
                  : isToday ? 'var(--accent)'
                  : 'var(--text)',
...
export default function AgentCalendarPage() {
  ...
  const updateMut = useMutation({
    mutationFn: ({ listingId, scheduledAt, status }: { listingId: string; scheduledAt: string; status: ScheduleStatus }) =>
      schedulesApi.updateStatus(listingId, scheduledAt, status),
    onSuccess: () => {
  ...
  const daySchedules = selectedDay
    ? monthSchedules.filter(s => new Date(s.scheduledAt).getDate() === selectedDay)
    : []
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: 20, alignItems: 'start' }}>
        ...
        <div className="card">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <span style={{ fontFamily: 'DM Serif Display, serif', fontSize: '1.1rem' }}>{monthName}</span>
          ...
          {/* Legend */}
          <div className="flex gap-4 mt-4" style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
        ...
        <div className="card" style={{ position: 'sticky', top: 80 }}>
          ...
          {daySchedules.length === 0 ? (
            <p style={{ color: 'var(--text-dim)', fontSize: '0.875rem' }}>
              {selectedDay ? 'No viewings scheduled for this day.' : 'Click a day on the calendar.'}
            </p>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {daySchedules.map((s, i) => (
                <div key={i} style={{
                  ...
                  <div style={{ fontWeight: 600, marginBottom: 4, fontSize: '0.9rem' }}>{s.listingName}</div>
                    ...
                    {s.status === 'Pending' && (
                      <>
                        <button className="btn btn-sm" style={{ background: 'var(--teal-dim)', color: 'var(--teal)', border: 'none', cursor: 'pointer' }}
                          onClick={() => updateMut.mutate({ listingId: s.listingId, scheduledAt: s.scheduledAt, status: 'Confirmed' })}>
                          <Check size={12} /> Confirm
                        </button>
                        <button className="btn btn-danger btn-sm"
                          onClick={() => updateMut.mutate({ listingId: s.listingId, scheduledAt: s.scheduledAt, status: 'Cancelled' })}>
                          <X size={12} /> Cancel
                        </button>
                      </>
                    )}
