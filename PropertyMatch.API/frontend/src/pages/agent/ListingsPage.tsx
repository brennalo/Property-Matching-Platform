...
function StatusBadge({ status }: { status: Listing['status'] }) {
  ...
  return (
    <span className={`badge ${map[status] ?? 'badge-grey'}`}>
      {icons[status]} {status === 'PendingPayment' ? 'Awaiting Payment' : status}
    </span>
  )
}
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        <h2 style={{ marginBottom: 20 }}>{initial ? 'Edit Listing' : 'New Listing'}</h2>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div className="form-grid">
            <div className="form-group">
              ...
              <input className="input" type="number" min={1} max={10} value={form.rooms}
                onChange={e => upd('rooms', e.target.value)} />
            </div>
            <div className="form-group">
              <label className="form-label">Bathrooms</label>
              <input className="input" type="number" min={1} max={10} value={form.toilets}
          ...
          <div className="form-grid">
            <div className="form-group">
              ...
              <input className="input" type="number" step="any" value={form.lat}
                onChange={e => upd('lat', e.target.value)} placeholder="e.g. 3.1478" />
            </div>
            <div className="form-group">
              <label className="form-label">Longitude</label>
              <input className="input" type="number" step="any" value={form.lng}
                onChange={e => upd('lng', e.target.value)} placeholder="e.g. 101.6953" />
            </div>
          </div>
          ...
          <div className="form-group">
            ...
            <select className="select" value={form.residencyType}
        ...
        <div className="flex gap-3 mt-5">
          ...
          <button className="btn btn-primary" onClick={() => onSave(form)}
            ...
            {loading ? <span className="spinner" /> : (initial ? 'Save Changes' : 'Create Listing')}
          </button>
        </div>
      </div>
    </div>
...
function ImageUploadModal({ listing, onClose }: { listing: Listing; onClose: () => void }) {
        ...
        {success ? (
          ...
          <>
            <div style={{
              ...
              <ImagePlus size={28} color="var(--text-muted)" style={{ marginBottom: 8 }} />
              <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem' }}>
                {files.length > 0 ? `${files.length} file(s) selected` : 'Click to select images (JPG, PNG, WebP · max 5MB each)'}
              </p>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 12 }}>
                {files.map((f, i) => (
                  <div key={i} style={{
                    ...
                    borderRadius: 6, fontSize: '0.78rem', color: 'var(--text-muted)',
                  }}>
                    {f.name}
                  </div>
                ))}
...
export default function AgentListingsPage() {
  ...
  const createMut = useMutation({
    ...
    onError: (e: any) => showToast(e.response?.data?.message ?? 'Failed to create listing', 'error'),
  })
  ...
  return (
    <div>
      ...
      {paymentStatus === 'cancelled' && (
        <div style={{ padding: '12px 16px', background: 'var(--red-dim)', border: '1px solid var(--red)', borderRadius: 'var(--radius)', color: 'var(--red)', marginBottom: 16, fontSize: '0.875rem' }}>
          ...
          {listings.map(l => (
            <div key={l.id} className="card">
              <div className="flex gap-4 items-start">
                {l.imageUrls[0] ? (
                  <img src={l.imageUrls[0]} alt={l.name}
                    style={{ width: 100, height: 72, objectFit: 'cover', borderRadius: 8, flexShrink: 0 }} />
                ) : (
                  <div style={{ width: 100, height: 72, background: 'var(--bg-input)', borderRadius: 8, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '2rem' }}>
                    🏠
                  </div>
                )}
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: '0.82rem', color: 'var(--text-muted)' }}>
                    {l.rooms} bed · {l.toilets} bath · {l.residencyType} · {l.address}
                  </div>
                  <div style={{ color: 'var(--accent)', fontFamily: 'DM Serif Display, serif', marginTop: 4 }}>
                ...
                <div className="flex gap-2" style={{ flexShrink: 0, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                  {l.status === 'PendingPayment' && (
                    <button className="btn btn-primary btn-sm"
                      ...
                      {payLoading === l.id ? <span className="spinner" /> : <><CreditCard size={13} /> Pay to Activate</>}
                    </button>
                  )}
                  ...
                  <button className="btn btn-ghost btn-sm"
                    onClick={() => { setEditTarget(l); setShowForm(false) }}>
                    <Pencil size={13} />
                  </button>
                  <button className="btn btn-danger btn-sm"
                    disabled={deleteMut.isPending}
                    onClick={() => { if (confirm(`Delete "${l.name}"?`)) deleteMut.mutate(l.id) }}>
                    <Trash2 size={13} />
                  </button>
                </div>
              </div>
      ...
      {imageTarget && (
        <ImageUploadModal listing={imageTarget} onClose={() => setImageTarget(null)} />
      )}
